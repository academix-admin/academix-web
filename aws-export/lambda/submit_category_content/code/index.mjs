...
export const handler = async (event) => {
  ...
  try {
    ...
    const { exists, allowed, is_public, group, registered_locale } = categoryCheck;
    ...
    // Content language is the creator's REGISTERED language (returned by get_category_exists), never
    // the client-sent `locale` — that's just whatever display language they're currently viewing in.
    const prompt = buildPrompt(categoryName, group, registered_locale, is_public);
