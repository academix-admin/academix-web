  ...
  return (
    <main className={`${styles.container} ${styles[`container_${theme}`]}`}>
      ...
      <div className={styles.innerBody}>
         ...
         {fetchedUserData && (<span>{fetchedUserData.usersNames}</span>)}
         {fetchedUserData && (<span>{fetchedUserData.usersEmail}</span>)}
         {fetchedUserData && (<span>{fetchedUserData.usersPhone}</span>)}
         {fetchedUserData && (<span>{fetchedUserData.usersUsername}</span>)}

         {!fetchLoading && !fetchedUserData && error && (<ErrorView text={error} buttonText="Try Again" onButtonClick={fetchUserData} />)}
         {!fetchLoading && !fetchedUserData && !error && (<NoResultsView text="No result" buttonText="Try Again" onButtonClick={fetchUserData} />)}
