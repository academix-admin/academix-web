...
function formatTimeWithTranslation(seconds: number): string {
...
function formatResponseTimeWithTranslation(time: number, t: (key: string, params?: any) => string): string {
  if (time < 1) {
...
function formatRank(rank: number, t: (key: string, params?: any) => string): string {
...
function QuizHistoryRow({ quiz, onClick, style }: { quiz: QuizHistory; onClick: () => void; style?: React.CSSProperties }) {
    ...
    <div className={styles.historyItemContainer} onClick={onClick} style={style}>
      ...
      <div className={styles.historyItem}>
        <div className={styles.historyMain}>
          ...
          <span className={styles.historyTime}>{formatDate(quiz.poolsMembersCreatedAt, lang)}</span>
        </div>
        ...
        <div className={styles.historyDetailsBottom}>
          <div className={`${applyTheme(styles, 'historyDetails')}`}>
            ...
            <span>
              ...
              {formatRank(quiz.poolsMembersRank, t)}
            </span>
            ...
            <span>
              ...
              {quiz.poolsCompletedQuestionTrackerTime > 0
                ? formatResponseTimeWithTranslation(quiz.poolsCompletedQuestionTrackerTime, t)
                : '0ms'}
            </span>
            ...
            <span className={styles.counter}>
              ...
              {quiz.poolsCompletedQuestionTrackerSize} / {quiz.challengeQuestionCount}
            </span>
          </div>
          {/* Amount */}
    ...
    <div className={styles.historyContainer}>
      ...
      <div className={styles.historyList}>
        {quizHistoryData.map((quiz, index) => (
          <QuizHistoryRow key={index} quiz={quiz} onClick={() => handleQuizClick(quiz)} />
        ))}
      </div>
      ...
      >
        {searchResults.map((r) => (
          <QuizHistoryRow key={r.data.poolsId} quiz={r.data} onClick={() => handleQuizClick(r.data)} style={{ padding: '0 12px' }} />
        ))}
      </SearchViewer>
