...
export default function QuizDetailsViewer({ topicsModel }: QuizDetailsViewerProps) {
  ...
  const handleCopyCode = async () => {
    ...
    try {
      await copyToClipboard(topicsModel.quizPool.poolsCode);
  ...
  const handleFollowToggle = async () => {
    if (!userData?.usersId || isFollowLoading) return;
    ...
    try {
      ...
      if (status === 'FollowStatus.followed') {
        setIsFollowed(true);
      } else if (status === 'FollowStatus.unfollowed') {
        setIsFollowed(false);
      }
    } catch (err) {
  ...
  return (
    <div className={styles.experienceContainer}>
      ...
      {topicsModel.quizPool?.poolsLocale && (<div className={styles.codeContainer} onClick={() => codeBottomController.open()}>
      ...
      <div className={`${styles.performanceItem} ${styles[`performanceItem_${theme}`]}`}>
        ...
        <div className={styles.creatorInfo}>
          ...
          {topicsModel.creatorId && userData?.usersId !== topicsModel.creatorId && (
            <button
              className={styles.followButton}
              onClick={handleFollowToggle}
              disabled={isFollowLoading}
            >
              {isFollowLoading ? <span className={styles.spinner}></span> : (isFollowed ? t('unfollow_text') : t('follow_text'))}
            </button>
          )}
        </div>
      </div>
