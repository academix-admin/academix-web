...
export default function WithdrawPage() {
  ...
  nav.title(t('page_title_withdraw'));
   ...
   * under 'withdraw-flow'. Two independent bugs: the wrong scope name (so the data was never cleared at
  ...
  usePageLifecycle(nav, {
    onExit: () => { StateStack.core.clearScope('withdraw-flow'); },
  });
  ...
  const handleWithdraw = async (userPin: string) => {
  ...
  return (
    <main className={`${applyTheme(styles, 'container')}`}>
      ...
      <Header title={t('withdraw_text')} theme={theme} onBack={goBack} />
