...
function formatDate(dateString: string): string {
  ...
  const formattedTime = date.toLocaleTimeString('en-US', timeOptions).toLowerCase().replace(' ', '');
  return `${formattedDate} at ${formattedTime}`;
}

function getTransactionTypeText(transactionType: string | undefined | null, t: (key: string) => string): string {
...
function getTransactionStatus(transaction: TransactionModel): string | null {
...
function getTransactionAmount(transaction: TransactionModel): number {
  switch (transaction.transactionType) {
    case 'TransactionType.top_up':
...
function getPaymentMethodName(paymentMethod: string | undefined | null, t: (key: string) => string): string {
...
function getTransactionStatusClass(status: string | undefined | null, t: (key: string) => string): string {
  switch (status) {
...
function getStatusClass(status: string | null, type: string | null): string {
  switch (status) {
    case 'TransactionStatus.success':
...
function getTransactionIcon(transactionType: string | undefined | null): React.JSX.Element {
  switch (transactionType) {
    case 'TransactionType.top_up':
...
function TransactionRow({
  ...
  return (
    <div className={`${styles.historyItemContainer}`} onClick={onClick} style={style}>
        ...
        <div className={styles.historyItem}>
          <div className={styles.historyMain}>
            <span className={`${applyTheme(styles, 'topicName')}`}>
              {getPaymentMethodName(transaction.paymentProfileSenderDetails?.paymentMethodDetails.paymentMethodChecker, t)}
              {' '}
              {getTransactionTypeText(transaction.transactionType, t)}
            </span>
          ...
          <div className={styles.historyContent}>
            <div className={styles.historyDetails}>
              ...
              >
                {capitalize(getTransactionStatusClass(getTransactionStatus(transaction), t))}
              </span>
            </div>
          </div>
        </div>
...
export default function PaymentTransactions({ onStateChange }: ComponentStateProps) {
  ...
  const matchesLocal = (m: TransactionModel, q: string) =>
    (getTransactionTypeText(m.transactionType, t) || '').toLowerCase().includes(q) ||
    ...
    <div className={styles.historyContainer}>
      ...
      <div className={styles.historyList}>
        {transactionModels.map((transaction, index) => (
          <TransactionRow
            ...
            showDivider={index < transactionModels.length - 1}
      ...
      >
        {searchResults.map((r, index) => (
          <TransactionRow
            key={r.data.transactionId}
            transaction={r.data}
            showDivider={index < searchResults.length - 1}
            onClick={() => handleTransactionClick(r.data)}
