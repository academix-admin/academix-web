...
const StatusSection = ({
  ...
  const containerClasses = {
    mobile: `${applyTheme(styles, 'mobileStatusContainer')}`,
    tablet: `${applyTheme(styles, 'tabletStatusContainer')}`,
    web: `${applyTheme(styles, 'webStatusContainer')}`
  };
  ...
  const StatusRow = ({ label, value, showTopBorder = false }: { label: string; value: string; showTopBorder?: boolean }) => (
    <div className={`${styles.statusRow} ${showTopBorder ? styles.statusRowWithBorder : ''} ${styles[`statusRow_${theme}`]}`}>
      ...
      <div className={`${applyTheme(styles, 'statusValue')}`}>
        {value}
      </div>
    </div>
  );
  ...
  return (
    <div className={containerClasses[size]}>
      <div className={`${applyTheme(styles, 'statusTable')}`}>
    ...
    <div className={`${applyTheme(styles, 'webQuizContainer')}`}>
      ...
      <div className={`${applyTheme(styles, 'webQuizHeader')}`}>
        ...
        <div className={styles.completionTextContainer}>
          <span className={`${applyTheme(styles, 'completionText')}`}>
            {t('completion_text')}
          </span>
        </div>
