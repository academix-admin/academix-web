...
export default function Quiz({ params }: { params: Promise<{ poolsId: string }> }) {
  ...
  const determineState = useCallback(() => {
    ...
    const isUserControlledState = endTimeFrom === 'tracker' || endTimeFrom === 'timer';
    if (isUserControlledState && pendingQuestions.length <= 0) {
      return;
    }
    ...
    if (pendingQuestions.length > 0 && quizState !== 'quizPlay') {
      setEndTimeFrom(null); // reset end time source when going back to play
