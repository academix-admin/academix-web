-- schema:   public
-- function: fetch_topics(p_locale, p_limit_by, p_after_topics, p_type, p_category_id, p_reviewer_tab)
-- identity + demographics derived server-side via gate_check, NOT client-sent.
-- generated from Supabase project iewqfmkngcgayxbbnpiz (read-only mirror)

CREATE OR REPLACE FUNCTION public.fetch_topics(p_locale text, p_limit_by integer, p_after_topics jsonb, p_type text, p_category_id uuid DEFAULT NULL::uuid, p_reviewer_tab text DEFAULT NULL::text)
 RETURNS SETOF jsonb
 LANGUAGE plpgsql
AS $function$
DECLARE
...
BEGIN
    ...
    WITH filtered_topics AS (
        SELECT
            ...
            tt.approval_status,
            (SELECT translation::uuid FROM translate(tt.topics_created_by, v_locale)) AS users_creator_id,
            (SELECT translation::uuid FROM translate(tt.topics_reviewed_by, v_locale)) AS users_reviewer_id
        ...
        WHERE
            (SELECT * FROM fetch_general_content_check(
                (SELECT translation::uuid FROM translate(tt.topics_created_by, v_locale)),
                p_user_id,
                (SELECT translation FROM translate(tt.topics_identity, v_locale)),
                (SELECT translation::uuid FROM translate(tt.topics_reviewed_by, v_locale)),
                (SELECT value FROM decontrol(tt.language_control, v_locale)),
                ...
                tt.topics_visible,
                (SELECT translation FROM translate(tt.approval_status, v_locale))
            )) = true
    ...
    SELECT
        jsonb_build_object(
            ...
            'topics_identity', (SELECT translation FROM translate(ft.topics_identity, v_locale)),
            'approval', (SELECT translation FROM translate(ft.approval_status, v_locale)),
            ...
            'age_control', (SELECT jsonb_agg(control) FROM build_control(ft.age_control, v_locale) AS control),
            'country_control', (SELECT jsonb_agg(control) FROM build_control(ft.country_control, v_locale) AS control),
            'language_control', (SELECT jsonb_agg(control) FROM build_control(ft.language_control, v_locale) AS control),
            'gender_control', (SELECT jsonb_agg(control) FROM build_control(ft.gender_control, v_locale) AS control)
        )
