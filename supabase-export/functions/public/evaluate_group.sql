-- schema:   public
-- function: evaluate_group(p_user_id uuid, p_group_id uuid, p_approval_checker text)
-- generated from Supabase project iewqfmkngcgayxbbnpiz (read-only mirror)

CREATE OR REPLACE FUNCTION public.evaluate_group(p_user_id uuid, p_group_id uuid, p_approval_checker text)
 RETURNS jsonb
 LANGUAGE plpgsql
AS $function$
DECLARE
...
BEGIN
    ...
    -- Update the category_group_table with the approval
    time := NOW()::TEXT;
    ...
    SET 
        ...
        category_group_reviewed_by = COALESCE(category_group_reviewed_by, '{}'::JSONB) || jsonb_build_object(locale, p_user_id),
        category_group_updated_at = time,
