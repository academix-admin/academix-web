-- schema:   public
-- function: get_user_topic_questions(p_user_id uuid, p_topic_id uuid, p_country text, p_locale text, p_gender text, p_age text, p_limit_by integer, p_after_question text)
-- generated from Supabase project iewqfmkngcgayxbbnpiz (read-only mirror)

CREATE OR REPLACE FUNCTION public.get_user_topic_questions(p_user_id uuid, p_topic_id uuid, p_country text, p_locale text, p_gender text, p_age text, p_limit_by integer, p_after_question text DEFAULT NULL::text)
 RETURNS SETOF jsonb
 LANGUAGE plpgsql
AS $function$
BEGIN
    ...
    EXECUTE format('
        SELECT
            jsonb_build_object(
                ...
                FROM get_all_question_options(%L, qt.questions_id, %L)),
        ...
        WHERE (((SELECT * FROM override_access_checker(qt.users_creator_id, %L, %L)) = true) OR 
        qt.users_creator_id = %L)
...
REVOKE EXECUTE ON FUNCTION public.get_user_topic_questions(p_user_id uuid, p_topic_id uuid, p_country text, p_locale text, p_gender text, p_age text, p_limit_by integer, p_after_question text) FROM PUBLIC, anon;
