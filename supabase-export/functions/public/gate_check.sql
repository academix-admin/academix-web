...
-- The feature gate always runs; NULL gender/age/country are handled tolerantly inside get_feature_status
-- (so a brand-new profile-less signup isn't blocked by null gender/age or an unresolved IP country),
-- while features_active + real controls (incl. language) still enforce. Region/compliance always applies.
