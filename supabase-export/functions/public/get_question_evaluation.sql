-- schema:   public
-- function: get_question_evaluation(p_question_id uuid, p_locale text)
-- generated from Supabase project iewqfmkngcgayxbbnpiz (read-only mirror)

CREATE OR REPLACE FUNCTION public.get_question_evaluation(p_question_id uuid, p_locale text)
 RETURNS jsonb
 LANGUAGE plpgsql
AS $function$
DECLARE
...
BEGIN
    SELECT jsonb_build_object(
        ...
        'options_table', (SELECT jsonb_agg(option) FROM get_option_evaluation(v_user_id, qt.questions_id, p_locale) AS option),
    ...
    WHERE
        (
            (SELECT permission_checker((SELECT translation::uuid FROM translate(qt.questions_created_by,p_locale)), v_user_id)) = TRUE
