-- schema:   public
-- function: evaluate_question(p_user_id uuid, p_question_id uuid, p_approval_checker text)
-- generated from Supabase project iewqfmkngcgayxbbnpiz (read-only mirror)

CREATE OR REPLACE FUNCTION public.evaluate_question(p_user_id uuid, p_question_id uuid, p_approval_checker text)
 RETURNS jsonb
 LANGUAGE plpgsql
AS $function$
DECLARE
...
BEGIN
    ...
    -- Update the questions_table with the approval
    time := NOW()::TEXT;
    ...
    SET 
        ...
        questions_reviewed_by = COALESCE(questions_reviewed_by, '{}'::JSONB) || jsonb_build_object(locale, p_user_id),
        questions_updated_at = time,
