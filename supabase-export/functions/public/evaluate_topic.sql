-- schema:   public
-- function: evaluate_topic(p_topic_id uuid, p_approval_checker text)
-- generated from Supabase project iewqfmkngcgayxbbnpiz (read-only mirror)

CREATE OR REPLACE FUNCTION public.evaluate_topic(p_topic_id uuid, p_approval_checker text)
 RETURNS jsonb
 LANGUAGE plpgsql
AS $function$
DECLARE
...
BEGIN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM topics_table
        WHERE topics_id = p_topic_id
          AND topics_identity ? locale
    ) THEN
        RETURN jsonb_build_object('status', NULL, 'code', 'approval_failure');
    END IF;
    ...
    SET
        ...
        topics_reviewed_by = COALESCE(topics_reviewed_by, '{}'::JSONB) || jsonb_build_object(locale, v_user_id),
