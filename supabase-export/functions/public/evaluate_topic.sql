-- schema:   public
-- function: evaluate_topic(p_user_id uuid, p_topic_id uuid, p_approval_checker text)
-- generated from Supabase project iewqfmkngcgayxbbnpiz (read-only mirror)

CREATE OR REPLACE FUNCTION public.evaluate_topic(p_user_id uuid, p_topic_id uuid, p_approval_checker text)
 RETURNS jsonb
 LANGUAGE plpgsql
AS $function$
DECLARE
...
BEGIN
    ...
    -- Update the topics_table with the approval
    time := NOW()::TEXT;
    sort := tsid(time::timestamp);
    UPDATE topics_table
    SET 
        approval_status = approval_status || jsonb_build_object(locale, p_approval_checker),
        topics_reviewed_by = COALESCE(topics_reviewed_by, '{}'::JSONB) || jsonb_build_object(locale, p_user_id),
        topics_updated_at = time,
